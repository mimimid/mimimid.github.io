...
처음에는 테마가 적용된 것만으로도 만족했는데, 막상 글을 작성하고 보니 개발일지 블로그라기보다는 GitHub README 문서 같은 느낌이 강했다.

깔끔하긴 했지만, 글을 계속 쌓아가기에는 조금 아쉬웠다. 기왕이면 이쁜 블로그가 좋으니까~
...
처음에는 Ruby 최신 버전이면 당연히 더 좋은 줄 알았다.

그런데 Chirpy 7.5 기준으로는 Ruby 4.x가 아니라 Ruby 3.x 계열이 필요했다.
...
아직 디자인을 내 스타일로 다듬은 건 아니지만, 개발일지 블로그로 쓸 기본 구조는 훨씬 좋아진 거 같네!
...
처음에는 단순히 테마만 바꾸면 될 줄 알았는데, 생각보다 구조 변경이 많았다.
