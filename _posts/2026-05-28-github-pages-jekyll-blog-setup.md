...
분명 내가 해결한 건 맞는데.. 다시 보면 가물가물.. 다시 공식 문서랑 구글 찾아보고 해서

이번에는 공부한 내용만 정리하는 블로그가 아니라, 실제로 개발하면서 겪은 오류나 설정 과정, 해결 흐름까지 같이 남기는 개발일지 블로그를 만들어보기로 했다.
...
다만 이 옵션은 기존 파일을 덮어쓸 수 있으니 조심해야 한다.<br>
이번에는 괜찮았지만, 기존에 작성해둔 파일이 많은 프로젝트였다면 `--force`를 쓰기보다는 임시 폴더에서 Jekyll 사이트를 만든 뒤 필요한 파일만 복사하는 방식이 더 안전했을 것 같다.

잘못 쓰면 내 파일도 같이 힘으로 밀어버릴 수 있음 주의**
...
그래서 IntelliJ를 완전히 종료한 뒤 다시 실행했더니 정상적으로 실행이 됐다!

역시 안 되면 일단 재시작부터 해보자.<br>
...
일단 페이지부터 열기 위해 단순한 형태의 `jekyll-theme-minimal` 테마를 적용해보기로 했다.
...
다행히 큰 문제는 아니었고, 테마마다 제공하는 layout 이름이 다른 것이였음
...
* `_site`는 직접 수정하지 않는다.
* IntelliJ 터미널이 명령어를 못 찾으면 프로그램을 껐다가 다시 켜보자.
